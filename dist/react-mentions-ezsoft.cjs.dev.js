...
var applyChangeToValue = function applyChangeToValue(value, plainTextValue, _ref, config) {
  ...
  var mappedSpliceStart = spliceStart; // mapPlainTextIndex(value, config, spliceStart, 'START')

  var mappedSpliceEnd = spliceEnd; //mapPlainTextIndex(value, config, spliceEnd, 'END')
...
var isRedo = function isRedo(event) {
...
var isUndo = function isUndo(event) {
  if (event.ctrlKey || event.metaKey) {
    switch (String.fromCharCode(event.which).toLowerCase()) {
      case 'z':
        if (!event.shiftKey) {
...
var makeTriggerRegex = function makeTriggerRegex(trigger) {
  ...
  if (trigger instanceof RegExp) {
    ...
    var escapedTriggerChar = trigger; // first capture group is the part to be replaced on completion
...
var MentionsInput = /*#__PURE__*/function (_React$Component) {
  ...
  function MentionsInput(_props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "executeOnChange", function (event) {
      _this.redo = [];
      _this.last != null && _this.undo.push(_this.last);
      _this.last = event.target.value;
    ...
    _defineProperty(_assertThisInitialized(_this), "doRedo", function () {
      if (_this.redo.length > 0) {
        ...
        _this.undo.push(_this.props.value);
    ...
    _this.undo = [];
    _this.redo = [];
    _this.ref = _props.value;
    _this.last = _props.value || '';
    _this.schedules = [];
    _this.state = {
      focusIndex: 0,
      selectionStart: null,
  ...
  _createClass(MentionsInput, [{
    ...
    value: function handlePaste(event) {
      ...
      if (!this.supportsClipboardActions(event) || !this.props.replaceMarkupOnPaste) {
        return;
      }
...
var Mention = function Mention(_ref) {
  ...
  return /*#__PURE__*/React__default.createElement("strong", _extends({}, styles, {
    className: className
  }), display);
};
